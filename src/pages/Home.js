...
const AiwithImage = () => {
  ...
  async function aiImageRun() {
    if (fileSelected) {
      ...
      const result = await model.generateContent([
        "'Gemini, your task is to identify whether the image that is provided is of a leaf or not. If you can not find any leaf then reply \'No leaf detected !! Try another image\'. If its a leaf then do not reply anything like leaf detected, rather identify if the leaf is healthy or not. If the leaf is healthy then reply \'Healthy leaf\'. If the leaf is not healthy and seems abnormal,then do not reply that the leaf is not normal or healthy or seems abnormal, rather try to figure out what are the diseases, deficiencies, or decay that is causing the abnormality in the leaf, and reply the exact name of the disease or deficiency that you found, do not reply the explanation of the disease or do not reply with any extra text just reply the name of the disease or deficiency. Another big point is I strictly do not want any text like leaf detected'", imageInlineData
      ]);
  ...
  const [showResponseText, setShowResponseText] = useState(false); // New state to control style visibility
    async function fetchResponse() {
      try {
        ...
        const prompt = `if "${aiResponse}" is \'Healthy leaf\', then reply,\'No need to worry, leaf seems healthy.\', else, provide a precautionary measures to prevent "${aiResponse}" leaf disease.`;
  ...
  return (
    ...
    alignItems: 'center'}}>
      <h1 style={{color:'#333333', fontFamily: 'sans-serif'}}>Crop Cure </h1>
      <h1 style={{margin: '0 0', fontSize: '80px'}}>🍃</h1>
      <h4 style={{color:'#333333'}}>Smart Crop Disease Detector App!</h4>
      <div style={{display: 'flex',flexDirection: 'row', margin: '30px 0px', gap: '20px', alignItems: 'center', justifyContent: 'center', gap:'20px'}}>
      <p style={{fontSize: '20px',fontStyle: 'oblique',fontWeight: '600'}}>Enter image file that contains picture of leaf to check for disease.</p>
      </div>
